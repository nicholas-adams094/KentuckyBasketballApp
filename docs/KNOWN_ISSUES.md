...
2. **Four portraits are crops of a team photograph**, not individual archival headshots:
   ...
   it is a crop of a group photograph. These four are also the only portraits that look
   soft at card size: their native crops are 67–87px against a median of 356px. Replace
   with verified individual portraits when they can be located.

3. **One player is shown as a jersey card rather than a photograph.** *Eric Allen* — no
...
4. **Two portraits post-date the era.** The only surviving official Kentucky headshots of
...
5. **Originals are low resolution.** 52 of 68 player originals are 420×560; several team
   ...
   Derivatives are now capped at 2× the native crop rather than upscaled 3×, so the
   archive no longer claims resolution it does not hold — but the underlying detail is
   unchanged. The correct fix is re-sourcing; see [PHOTO_PIPELINE.md](PHOTO_PIPELINE.md).
...
6. **Two players appear in both the starting five and the rotation list** — Keith Bogans
...
7. **Tournament round wording is inconsistent between seasons.** 1997–98 through 2001–02
...
8. **The archive stores only per-game rates** — no totals, no shooting splits, no
...
9. **`conferenceFinish` is prose and its wording varies.** Deriving facts from it is
...
10. **No real assistive-technology testing.** Accessibility is verified against the
   accessibility tree and by automated checks, not with VoiceOver, NVDA or JAWS. See
   [ACCESSIBILITY.md](ACCESSIBILITY.md).

11. **No axe-core sweep.** The a11y checks are hand-written and targeted rather than a
    generic rule engine, so they catch what they were written to catch.

12. **No service worker.** The build is fully static and works offline once cached by the
    browser, and the web manifest is in place, but there is no explicit precache, so a
    first visit still requires a network.

13. **Playwright is pinned to `~1.56.1`** so it matches the browser build available in
